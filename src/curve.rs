...
// Solves for t such that the L component of the Bézier curve is equal to target_l
...
pub fn generate_oklab_samples(p1: Oklab, desired_l_values: &[f32]) -> Vec<Oklab> {
    let start_point = Oklab { l: 1.0, a: 0.0, b: 0.0 }; // Pure white
    let end_point = Oklab { l: 0.0, a: 0.0, b: 0.0 }; // Pure black
    let control_point = p1;

    desired_l_values
        .iter()
        .filter_map(|&l| find_t_for_l(l, start_point, p1, end_point).map(|t| {
